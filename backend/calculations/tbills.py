...
class TBillsCalculator:
    """Comprehensive T-Bills valuation calculator."""
    ...
    def calculate_all_metrics(self, inputs: Dict, benchmark_yield: Optional[float] = None,
        ...
        if maturity_date and settlement_date:
            ...
            days_to_maturity = 90  # Default 90-day T-Bill
            results['days_to_maturity'] = days_to_maturity
        ...
        if discount_rate and face_value:
            ...
            if not purchase_price:
                purchase_price = self._calculate_purchase_price(face_value, discount_rate, days_to_maturity)
            ...
            results['fair_value'] = round_money(purchase_price)
            results['market_value'] = round_money(purchase_price)
            ...
            # Percentage Return
            percentage_return = hpy * 100
            results['percentage_return'] = round_percentage(percentage_return)
        ...
        if benchmark_yield is not None:
            # Benchmark Spread
            if 'bond_equivalent_yield' in results:
            ...
            # Valuation using Benchmark Yield Curve
            if face_value and days_to_maturity:
                benchmark_valuation = self._calculate_benchmark_valuation(
                    face_value, benchmark_yield, days_to_maturity
                )
                results['benchmark_valuation'] = round_money(benchmark_valuation)
        
        # Real Yield (Inflation Adjusted)
        if inflation_rate is not None and 'bond_equivalent_yield' in results:
