...
import { isStepCompleted as checkStepCompleted, farthestAllowedIndex as getFarthestAllowedIndex, markStepCompleted, autoMarkStepIfCompleted, resetWorkflowProgress } from '@/utils/workflowProgress.js'
...
// ---- Custom step completion (state-driven) ----
function isStepComplete(tab) {
  ...
  return checkStepCompleted(tab, steps.value, state, sessionId)
}
...
const farthestAllowedIndex = computed(() => {
  ...
  const sessionId = activeSession.value?.id
  return getFarthestAllowedIndex(steps.value, state, sessionId)
})
...
// ===== MODIFIED readFileData to use backend API =====
...
async function continueAfterUpload() {
  ...
  // Auto-mark upload step as completed if criteria met
  const state = {
    rawDataLength: rawData.value.length,
    mappingApplied: mappingApplied.value,
    allColumnsMapped: missingColumns.value.length === 0,
  ...
  autoMarkStepIfCompleted('upload', state, activeSession.value?.id)
...
async function continueAfterCleaning() {
  ...
  // Auto-mark cleaning step as completed if criteria met
  const state = {
    rawDataLength: rawData.value.length,
    mappingApplied: mappingApplied.value,
    allColumnsMapped: missingColumns.value.length === 0,
  ...
  autoMarkStepIfCompleted('cleaning', state, activeSession.value?.id)
  try {
