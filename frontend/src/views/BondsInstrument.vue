              ...
              <div v-else>
                ...
                <div class="summary-cards">
                  ...
                  <div class="summary-card rate" @click="showFormula('Average Rate')">
                    <div class="card-label">Avg Coupon Rate</div>
                    <div class="card-value">{{ calculations.avgCouponRate || 0 }}%</div>
                  </div>
            ...
            <v-card-text>
              ...
              <div class="summary-grid">
                ...
                <div class="summary-section card-panel">
                  ...
                  <p><strong>Average coupon rate:</strong> {{ calculations.avgCouponRate || 0 }}%</p>
                  <p><strong>Weighted average coupon:</strong> {{ calculations.weightedAvgCoupon || 0 }}%</p>
                  <p><strong>Average YTM:</strong> {{ calculations.avgYTM || 0 }}%</p>
                  <p><strong>Duration:</strong> {{ calculations.duration || 0 }} years</p>
...
const { fredFilters, filterOptions, loadFilterOptions, fetchBenchmark } = useFredMarket('10Y') // default for bonds
...
const portfolioAvgRate = computed(() => calculations.value.avgCouponRate || 0)

const effectiveMaturity = computed(() => {
  if (selectedMaturityOption.value === '__custom__') return customMaturityInput.value || '10Y'
...
function openInstrumentDataExcel() {
  ...
  const calcRow = {
    ...
    'Average Coupon Rate': calculations.value.avgCouponRate || 0,
    'Weighted Average Coupon': calculations.value.weightedAvgCoupon || 0,
    'Total Annual Income': calculations.value.totalAnnualIncome || 0,
    'Average YTM': calculations.value.avgYTM || 0,
      ...
      if (activeSession.value?.id) {
        try {
          ...
          const summaryRow = {
            ...
            'Avg Coupon Rate': calculations.value.avgCouponRate || 0,
...
function onMaturitySelectChange() {
  if (selectedMaturityOption.value !== '__custom__') {
    ...
    fredFilters.value.maturity = customMaturityInput.value || '10Y'
  }
  onFredFilterChange()
}
  ...
  if (instrumentSummary.value.rows.length > 0) {
    ...
    if (rows.length > 0) {
      ...
      rows.forEach(row => {
        ...
        const rate = parseFloat(row['Avg Coupon Rate'] || row['Avg Rate'] || 0)
...
const reportPreviewData = computed(() => {
  ...
  if (instrumentsData.length === 0 && instrumentSummary.value.rows.length > 0) {
    ...
    for (const [type, rows] of Object.entries(grouped)) {
      ...
      rows.forEach(row => {
        const value = parseFloat(row['Total Value'] || row['Calculated Value'] || 0)
        const rate = parseFloat(row['Avg Coupon Rate'] || 0)
        if (!isNaN(value)) totalValue += value
        if (!isNaN(rate)) totalAvgRate += rate
      })
...
function formatMetricName(key) {
  const names = {
    totalValue: 'Total Value', instrumentCount: 'Count', avgCouponRate: 'Avg Coupon Rate',
    weightedAvgCoupon: 'Weighted Avg Coupon', totalAnnualIncome: 'Total Annual Income',
    avgYTM: 'Avg YTM', duration: 'Duration'
  }
...
function formatMetricValue(key, value) {
  if (typeof value === 'number') {
    if (key.includes('Value') || key.includes('Income') || key.includes('Price'))
      return `$${value.toLocaleString()}`
    if (key.includes('Rate') || key.includes('Yield') || key.includes('Coupon'))
      return `${value}%`
    return value.toLocaleString()
  }
  return value
    ...
    <tbody>
      ${report.instruments.map(inst => `
        <tr>
          ...
          <td>${inst.calculations.avgCouponRate || 0}%</td>
          <td>${inst.calculations.weightedAvgCoupon || 0}%</td>
        </tr>
      `).join('')}
