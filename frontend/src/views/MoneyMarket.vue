  ...
  <FixedLayout>
    <div class="instrument-page">
      ...
      <div class="tab-content">
        ...
        <div v-if="activeTab === 'upload'" class="content-card">
          <v-card>
            ...
            <v-card-text>
              ...
              <!-- ===== CLEAN MAPPING DIALOG – NO EXTRAS, NO TICKS ===== -->
              <v-dialog v-model="showMappingDialog" max-width="650px">
                <v-card>
                  ...
                  <v-card-text class="mapping-dialog-body">
                    ...
                    <!-- Saved Mappings link (kept for convenience) -->
              ...
              <!-- Saved Mappings Popup (unchanged) -->
        ...
        <!-- ===== VISUALIZATIONS ===== (unchanged, with timeout) -->
...
const { fredFilters, filterOptions, countryItems, currencyItems, maturityItems, loadFilterOptions, applyCountryDefaults, fetchBenchmark } = useFredMarket('1Y')
...
const countryOptions = computed(() => {
  if (countryItems.value?.length) {
    return countryItems.value.map(c => ({ value: c.code, label: c.name }))
  }
  return []
})
...
const currencyOptions = computed(() => {
  if (currencyItems.value?.length) {
    return currencyItems.value.map(c => ({ value: c.code, label: c.name || c.code }))
  }
  return []
})

const INSTRUMENT_MATURITIES = {
  'money-market': ['1M', '3M', '6M', '1Y'],
  bonds: ['2Y', '5Y', '10Y', '30Y'],
  tbills: ['4W', '8W', '13W', '26W', '52W']
}

const maturityOptions = computed(() => {
  const allowed = INSTRUMENT_MATURITIES[instrumentType.value] || []
  const fromApi = (maturityItems.value || []).map(m => ({
    value: m.code,
    label: m.name || m.code
  }))
  if (fromApi.length) {
    const filtered = allowed.length
      ? fromApi.filter(opt => allowed.includes(opt.value))
      : fromApi
    return filtered.length ? filtered : fromApi
  }
  return allowed.map(code => ({ value: code, label: code }))
})

// ===== FUNCTIONS (unchanged, except fetchYieldCurve with timeout) =====
...
function parseMaturityToYears(mat) {
  const s = String(mat || '')
  const match = s.match(/^(\d+(?:\.\d+)?)([YMW])$/i)
  if (match) {
    const num = parseFloat(match[1])
    const unit = match[2].toUpperCase()
    if (unit === 'M') return num / 12
    if (unit === 'W') return num / 52
    if (unit === 'Y') return num
  }
  return parseFloat(s) || 0
}

function parseMaturityToAxisValue(code) {
  const match = String(code || '').match(/^(\d+(?:\.\d+)?)([YMW])$/i)
  if (!match) return parseFloat(code) || 0
  return parseFloat(match[1])
}

function getMaturityUnit(code) {
  const match = String(code || '').match(/([YMW])$/i)
  return match ? match[1].toUpperCase() : 'Y'
}
...
async function loadHistoryFile(item) {
  if (confirm(`Load ${item.name}? Current unsaved data will be lost.`)) {
    ...
    applyCurrentMapping()
    // Show preview after loading from history
...
function handleFileUpload(e) {
  ...
  if (file) {
    ...
    setTimeout(() => readFileData(file), 50)
  }
}
...
function handleProcessSheet(sheetName, sheetData, sheetHeaders) {
  ...
  // Close viewer and proceed to mapping with selected sheet data
  showWorkbookViewer.value = false
  rawData.value = sheetData
  headers.value = sheetHeaders
  // Trigger the existing mapping workflow
  applyCurrentMapping()
}
...
function processInstrumentData(sheetName, data, instrumentType) {
  ...
  // Build dynamic columns from this result
...
function calculateAllInstruments(instruments, sheetName, instrumentType) {
  ...
  // Update Instrument Summary with all instruments
  ...
  // Append all to Portfolio Summary
...
function runInstrumentCalculations(data, instrumentType) {
  // This is a placeholder - integrate with existing calculation logic
  // For now, return basic summary based on data
  const results = {}
...
function handleSheetSelection(sheetData) {
  ...
  if (sheetData && sheetData.data) {
    ...
    columnMapping.value = autoMatchColumns(fileColumns.value, requiredColumns.value, columnVariations.value)
    applyCurrentMapping()
    // Don't auto-show preview - only show after explicit upload or history load
...
// ===== CALCULATIONS – dynamic column detection =====
async function calculateMetrics() {
  if (!cleanedData.value.length) return

  const firstRow = cleanedData.value[0] || {}
  const availableCols = Object.keys(firstRow)

  const findColumn = (possibleNames) => {
    for (const name of possibleNames) {
      const exact = availableCols.find(c => c === name)
      if (exact) return exact
    }
    for (const name of possibleNames) {
      const partial = availableCols.find(c => c.toLowerCase().includes(name.toLowerCase()))
      if (partial) return partial
    }
    return null
  }

  const instrumentCol = findColumn(['Instrument', 'BondName', 'TBillName', 'Security', 'Name', 'Description']) || availableCols[0]
  const amountCol = findColumn(['Amount', 'FaceValue', 'Face Value', 'Principal', 'Value', 'Notional', 'Nominal', 'Par Value', 'Investment']) || availableCols[1]
  const rateCol = findColumn(['Rate', 'InterestRate', 'Interest Rate', 'CouponRate', 'Coupon Rate', 'DiscountRate', 'Discount Rate', 'Yield', 'YTM']) || availableCols[2]
  ...
  if (instrumentCol) {
    const uniqueNames = new Set(cleanedData.value.map(row => row[instrumentCol]).filter(v => v && v !== 'N/A' && v !== '' && v !== undefined))
    uniqueInstrumentsCount = uniqueNames.size || cleanedData.value.length
  } else {
    uniqueInstrumentsCount = cleanedData.value.length
  }

  if (instrumentType.value === 'money-market') {
    const totalValue = cleanedData.value.reduce((s, r) => s + getNumber(r[amountCol]), 0)
    const totalRate = cleanedData.value.reduce((s, r) => s + getNumber(r[rateCol]), 0)
    const weightedSum = cleanedData.value.reduce((s, r) => s + (getNumber(r[rateCol]) * getNumber(r[amountCol])), 0)
    const avgRateVal = totalRate / (cleanedData.value.length || 1)
    calculations.value = {
      totalValue,
      instrumentCount: uniqueInstrumentsCount,
      avgRate: avgRateVal.toFixed(2),
      weightedAvgRate: totalValue > 0 ? (weightedSum / totalValue).toFixed(2) : '0.00',
    ...
    const totalYield = cleanedData.value.reduce((s, r) => s + getNumber(r[yieldCol]), 0)
    const avgCoupon = totalRate / (cleanedData.value.length || 1)
    const avgYieldVal = totalYield / (cleanedData.value.length || 1)
    calculations.value = {
      totalValue,
      instrumentCount: uniqueInstrumentsCount,
      avgCouponRate: avgCoupon.toFixed(2),
      weightedAvgCoupon: totalValue > 0 ? (weightedSum / totalValue).toFixed(2) : '0.00',
    ...
    const totalRate = cleanedData.value.reduce((s, r) => s + getNumber(r[rateCol]), 0)
    const weightedSum = cleanedData.value.reduce((s, r) => s + (getNumber(r[rateCol]) * getNumber(r[amountCol])), 0)
    const avgDiscount = totalRate / (cleanedData.value.length || 1)
    ...
    calculations.value = {
      ...
      weightedAvgDiscount: totalValue > 0 ? (weightedSum / totalValue).toFixed(2) : '0.00',
      totalDiscount: discountAmount.toFixed(2),
      effectiveYield: price > 0 ? ((Math.pow(1 + discountAmount / price, 365 / 91) - 1) * 100).toFixed(2) : '0.00',
      bondEquivalentYield: price > 0 ? ((discountAmount / price) * (365 / 91) * 100).toFixed(2) : '0.00',
      discountYield: totalValue > 0 ? ((discountAmount / totalValue) * (360 / 91) * 100).toFixed(2) : '0.00',
      moneyMarketYield: price > 0 ? ((discountAmount / price) * (360 / 91) * 100).toFixed(2) : '0.00',
      pricePer100: (100 * (1 - (avgDiscount / 100) * (91 / 360))).toFixed(2),
      totalPurchasePrice: price.toFixed(2),
      avgInvestment: (cleanedData.value.length > 0 ? (price / cleanedData.value.length).toFixed(2) : '0.00'),
      holdingPeriodYield: price > 0 ? ((discountAmount / price) * 100).toFixed(2) : '0.00',
      annualizedYield: price > 0 ? ((discountAmount / price) * (365 / 91) * 100).toFixed(2) : '0.00',
...
// ===== FRED / VISUALIZATIONS – with timeout =====
...
async function fetchYieldCurve() {
  ...
  if (!country) {
    yieldCurveError.value = 'Please select a country.'
    return
  }
  if (!maturity) {
  ...
  try {
    ...
    if (response.success && response.data?.maturities?.length) {
    ...
    if (err.name === 'AbortError') {
      yieldCurveError.value = 'Request timed out after 10 seconds. Please try again.'
    } else {
...
// ===== SAVED MAPPINGS TEMPLATES (unchanged) =====
...
async function loadSavedData() {
  ...
  if (wf) {
    loaded = applyWorkflowToPage(wf, {
      ...
      yieldCurveData, fredFilters, completedSteps, showPreview
    })
...
// ===== REPORT GENERATION (unchanged – included for completeness) =====
...
watch(() => activeTab.value, async (newTab) => {
  if (newTab === 'visualizations' && hasCleanedData.value && !yieldCurveLoading.value) {
    if (!effectiveCountry.value && countryOptions.value.length) {
      selectedCountryOption.value = countryOptions.value[0].value
      fredFilters.value.country = countryOptions.value[0].value
...
onMounted(async () => {
  ...
  if (!selectedCountryOption.value && countryOptions.value.length) {
    selectedCountryOption.value = countryOptions.value[0].value
    fredFilters.value.country = countryOptions.value[0].value
    applyCountryDefaults()
    if (fredFilters.value.currency) selectedCurrencyOption.value = fredFilters.value.currency
  }
  if (!effectiveMaturity.value) {
    const def = defaultMaturityForInstrument()
    selectedMaturityOption.value = def
...
<style scoped>
/* ===== STYLES (unchanged, with additions for mapping dialog) ===== */
...
.filter-group { position: relative; display: flex; flex-direction: column; flex: 1; min-width: 150px; }
.filter-select-wrapper { position: relative; width: 100%; }
...
.filter-select-wrapper .filter-arrow { position: absolute; right: 12px; top: 50%; transform: translateY(-50%); pointer-events: none; font-size: 14px; color: #0B2044; font-weight: bold; line-height: 1; }
.header-right { display: flex; align-items: center; gap: 12px; }
.btn-save-session { background: linear-gradient(135deg, #0B2044, #1E88E5); color: white; border: none; padding: 8px 14px; border-radius: 8px; font-size: 13px; font-weight: 600; cursor: pointer; display: inline-flex; align-items: center; gap: 6px; }
.btn-save-session:hover { transform: translateY(-1px); box-shadow: 0 4px 10px rgba(11,32,68,0.25); }
